...
public class DemoNoSQLTableEvents extends DemoNoSQLTableBase {
    private static final String LOG_TAG = DemoNoSQLTableEvents.class.getSimpleName();
    ...
    public class DemoGetWithPartitionKey extends DemoNoSQLOperationBase {
        ...
        public boolean executeOperation() throws AmazonClientException {
            ...
            result = mapper.load(EventsDO.class, AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());
        ...
        public List<DemoNoSQLResult> getNextResultGroup() {
            ...
            results.add(new DemoNoSQLEventsResult(result));
    ...
    public class DemoScanWithFilter extends DemoNoSQLOperationBase {
        ...
        private PaginatedScanList<EventsDO> results;
        private Iterator<EventsDO> resultsIterator;
        ...
        public boolean executeOperation() {
            ...
            filterExpressionAttributeNames.put("#currentAmount", "currentAmount");

            final Map<String, AttributeValue> filterExpressionAttributeValues = new HashMap<>();
            filterExpressionAttributeValues.put(":MincurrentAmount",
                new AttributeValue().withN("1111500000"));
            final DynamoDBScanExpression scanExpression = new DynamoDBScanExpression()
                .withFilterExpression("#currentAmount > :MincurrentAmount")
                .withExpressionAttributeNames(filterExpressionAttributeNames)
                .withExpressionAttributeValues(filterExpressionAttributeValues);

            results = mapper.scan(EventsDO.class, scanExpression);
            if (results != null) {
     ...
     */
    private static List<DemoNoSQLResult> getNextResultsGroupFromIterator(final Iterator<EventsDO> resultsIterator) {
        if (!resultsIterator.hasNext()) {
            return null;
        }
        ...
        do {
            ...
            final EventsDO item = resultsIterator.next();
            // Add the item to a group of results that will be displayed later.
            resultGroup.add(new DemoNoSQLEventsResult(item));
            itemsRetrieved++;
    ...
    public void insertSampleData() throws AmazonClientException {
        ...
        final EventsDO firstItem = new EventsDO();

        firstItem.setUserId(AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());
        firstItem.setContributors(DemoSampleDataGenerator.getSampleList());
        firstItem.setCurrentAmount(DemoSampleDataGenerator.getRandomSampleNumber());
        firstItem.setMembers(DemoSampleDataGenerator.getSampleList());
        firstItem.setTarget(DemoSampleDataGenerator.getRandomSampleNumber());
        firstItem.setTitle(
        ...
        final EventsDO[] items = new EventsDO[SAMPLE_DATA_ENTRIES_PER_INSERT-1];
        for (int count = 0; count < SAMPLE_DATA_ENTRIES_PER_INSERT-1; count++) {
            final EventsDO item = new EventsDO();
            item.setUserId(DemoSampleDataGenerator.getRandomSampleString("userId"));
            item.setContributors(DemoSampleDataGenerator.getSampleList());
            item.setCurrentAmount(DemoSampleDataGenerator.getRandomSampleNumber());
            item.setMembers(DemoSampleDataGenerator.getSampleList());
            item.setTarget(DemoSampleDataGenerator.getRandomSampleNumber());
    ...
    public void removeSampleData() throws AmazonClientException {
        ...
        final EventsDO itemToFind = new EventsDO();
        itemToFind.setUserId(AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());

        final DynamoDBQueryExpression<EventsDO> queryExpression = new DynamoDBQueryExpression<EventsDO>()
            .withHashKeyValues(itemToFind)
            .withConsistentRead(false)
            .withLimit(MAX_BATCH_SIZE_FOR_DELETE);

        final PaginatedQueryList<EventsDO> results = mapper.query(EventsDO.class, queryExpression);

        Iterator<EventsDO> resultsIterator = results.iterator();
        ...
        final List<EventsDO> batchOfItems = new LinkedList<EventsDO>();
